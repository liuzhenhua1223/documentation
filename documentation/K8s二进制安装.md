...
|              |                |              |            |                                                              |
| ------------ | -------------- | ------------ | ---------- | ------------------------------------------------------------ |
| k8s-master01 | 10.255.187.175 | 11.11.11.120 | centos-7.6 | kube-apiserver,kube-controller-manager,kube-scheduler,kubelet,kube-proxy,docker,etcd,nginx,keepalived |
| k8s-master02 | 10.255.187.145 | 11.11.11.223 | centos-7.6 | kube-apiserver,kube-controller-manager,kube-scheduler,kubelet,kube-proxy,docker,etcd,nginx,keepalived |
| k8s-master03 | 10.255.187.50  | 11.11.11.184 | centos-7.6 | kube-apiserver,kube-controller-manager,kube-scheduler,kubelet,kube-proxy,docker,etcd,nginx,keepalived |
| worker01     | 10.255.187.68  | 11.11.11.29  | centos-7.6 | kubelet,kube-proxy,docker,etcd                               |
| worker02     | 10.255.187.205 | 11.11.11.205 | centos-7.6 | kubelet,kube-proxy,docker,etcd                               |

## 注意：

如果使用的是虚拟机则需要配置**1.2**步骤具体如下：

### 修改网卡配置

1. 使用命令 **ip addr** 查看 linux 是否有 eth0 网卡

2. 修改ifcfg-ens33将里面的NAME和DEVICE项修改为eth0,ONBOOT修改魏`yes
...
# 更改网卡的UUID
ssh root@11.11.11.120 "nmcli con delete uuid 708a1497-2192-43a5-9f03-2ab936fb3c44;nmcli con add type ethernet ifname eth0 con-name eth0;nmcli con up eth0"
ssh root@11.11.11.223 "nmcli con delete uuid 708a1497-2192-43a5-9f03-2ab936fb3c44;nmcli con add type ethernet ifname eth0 con-name eth0;nmcli con up eth0"
ssh root@11.11.11.184 "nmcli con delete uuid 708a1497-2192-43a5-9f03-2ab936fb3c44;nmcli con add type ethernet ifname eth0 con-name eth0;nmcli con up eth0"
ssh root@11.11.11.29 "nmcli con delete uuid 708a1497-2192-43a5-9f03-2ab936fb3c44;nmcli con add type ethernet ifname eth0 con-name eth0;nmcli con up eth0"
ssh root@11.11.11.205 "nmcli con delete uuid 708a1497-2192-43a5-9f03-2ab936fb3c44;nmcli con add type ethernet ifname eth0 con-name eth0;nmcli con up eth0"

# 参数解释
# 
# ssh ssh root@11.11.11.120
# 使用SSH登录到IP为11.11.11.120的主机，使用root用户身份。
# 
...
# 修改静态的IPv4地址
ssh root@192.168.71.154 "nmcli con mod eth0 ipv4.addresses 11.11.11.120/24; nmcli con mod eth0 ipv4.gateway  192.168.71.1; nmcli con mod eth0 ipv4.method manual; nmcli con mod eth0 ipv4.dns "8.8.8.8"; nmcli con up eth0"
ssh root@192.168.71.156 "nmcli con mod eth0 ipv4.addresses 11.11.11.223/24; nmcli con mod eth0 ipv4.gateway  192.168.71.1; nmcli con mod eth0 ipv4.method manual; nmcli con mod eth0 ipv4.dns "8.8.8.8"; nmcli con up eth0"
ssh root@192.168.71.164 "nmcli con mod eth0 ipv4.addresses 192.168.0.33/24; nmcli con mod eth0 ipv4.gateway  192.168.71.1; nmcli con mod eth0 ipv4.method manual; nmcli con mod eth0 ipv4.dns "8.8.8.8"; nmcli con up eth0"
ssh root@192.168.71.166 "nmcli con mod eth0 ipv4.addresses 11.11.11.29/24; nmcli con mod eth0 ipv4.gateway  192.168.71.1; nmcli con mod eth0 ipv4.method manual; nmcli con mod eth0 ipv4.dns "8.8.8.8"; nmcli con up eth0"
ssh root@192.168.71.167 "nmcli con mod eth0 ipv4.addresses 11.11.11.205/24; nmcli con mod eth0 ipv4.gateway  192.168.71.1; nmcli con mod eth0 ipv4.method manual; nmcli con mod eth0 ipv4.dns "8.8.8.8"; nmcli con up eth0"
...
# 参数解释
# 
# ssh root@192.168.71.154
# 使用SSH登录到IP为192.168.71.154的主机，使用root用户身份。
# 
# "nmcli con mod eth0 ipv4.addresses 11.11.11.120/24"
# 修改eth0网络连接的IPv4地址为11.11.11.120，子网掩码为 24。
# 
# "nmcli con mod eth0 ipv4.gateway 192.168.71.1"
# 修改eth0网络连接的IPv4网关为192.168.71.1。
# 
...
# 没有IPv6选择不配置即可
ssh root@11.11.11.120 "nmcli con mod eth0 ipv6.addresses fc00:43f4:1eea:1::10; nmcli con mod eth0 ipv6.gateway fc00:43f4:1eea:1::1; nmcli con mod eth0 ipv6.method manual; nmcli con mod eth0 ipv6.dns "2400:3200::1"; nmcli con up eth0"
ssh root@11.11.11.223 "nmcli con mod eth0 ipv6.addresses fc00:43f4:1eea:1::20; nmcli con mod eth0 ipv6.gateway fc00:43f4:1eea:1::1; nmcli con mod eth0 ipv6.method manual; nmcli con mod eth0 ipv6.dns "2400:3200::1"; nmcli con up eth0"
...
ssh root@11.11.11.29 "nmcli con mod eth0 ipv6.addresses fc00:43f4:1eea:1::40; nmcli con mod eth0 ipv6.gateway fc00:43f4:1eea:1::1; nmcli con mod eth0 ipv6.method manual; nmcli con mod eth0 ipv6.dns "2400:3200::1"; nmcli con up eth0"
ssh root@11.11.11.205 "nmcli con mod eth0 ipv6.addresses fc00:43f4:1eea:1::50; nmcli con mod eth0 ipv6.gateway fc00:43f4:1eea:1::1; nmcli con mod eth0 ipv6.method manual; nmcli con mod eth0 ipv6.dns "2400:3200::1"; nmcli con up eth0"

# 参数解释
# 
# ssh root@11.11.11.120
# 通过SSH连接到IP地址为11.11.11.120的远程主机，使用root用户进行登录。
# 
...
cat >> /etc/hosts << EOF
11.11.11.120 k8s-master01
11.11.11.223 k8s-master02
11.11.11.184 k8s-master03
11.11.11.29 k8s-node01
11.11.11.148 k8s-node02
EOF

ssh-keygen -f /root/.ssh/id_rsa -N ''  -t rsa -b 2048

for i in 120 223 184 29 148;do ssh-copy-id -i /root/.ssh/id_rsa.pub 11.11.11.$i;done
...
```apl
for i in 120 223 184 29 148; do
  ssh root@11.11.11.$i 'mkdir -p /etc/yum.repos.d/bak'
  ssh root@11.11.11.$i 'mv /etc/yum.repos.d/* /etc/yum.repos.d/bak/' ;done

curl -o /etc/yum.repos.d/CentOS-Base.repo http://mirrors.aliyun.com/repo/Centos-7.repo

for i in 120 223 184 29 148;do ssh 11.11.11.$i "wget -O /etc/yum.repos.d/CentOS-Base.repo http://mirrors.aliyun.com/repo/Centos-7.repo  && wget -O /etc/yum.repos.d/epel.repo http://mirrors.aliyun.com/repo/epel-7.repo";done
...
```apl
for i in 120 223 184 29 148;do ssh 11.11.11.$i "yum update -y && yum -y install  wget psmisc vim net-tools nfs-utils telnet yum-utils device-mapper-persistent-data lvm2 git tar curl && yum -y install createrepo yum-utils wget epel* ";done
...
echo "nf_defrag_ipv4" | sudo tee /etc/modules-load.d/nf_defrag_ipv4.conf
echo "nf_defrag_ipv6" | sudo tee /etc/modules-load.d/nf_defrag_ipv6.conf
...
我通过登录对应的Github地址，将对应Version版本的包下载保存到了本地
```

![image-20241125095026590](https://github.com/liuzhenhua1223/2024-10-image/blob/master//computernetworks/image-20241125095026590.png?raw=true)

## 安装Containerd作为Runtime(推荐)

```apl
#下载地址CNI
#https://github.com/containernetworking/plugins/releases/
# wget 下载地址https://mirrors.chenby.cn/https://github.com/containernetworking/plugins/releases/download/v1.3.0/cni-plugins-linux-amd64-v1.3.0.tgz
cd /lzh
#创建cni插件所需目录
...
生成自签名证书：
openssl x509 -req -days 365 -in calico.csr -signkey calico.key -out calico.crt
生成CA私钥
openssl genpkey -algorithm RSA -out ca-key.pem
生成ca证书
openssl req -new -x509 -key ca-key.pem -out ca.pem -days 365 -subj "/CN=calico-ca"

移动证书文件
sudo mkdir -p /etc/calico/certs
sudo mv calico.key /etc/calico/certs/key.pem
sudo mv calico.crt /etc/calico/certs/cert.pem
sudo mv ca.pem /etc/calico/certs/ca.pem
sudo mv ca-key.pem /etc/calico/certs/
...

{
  ...
  "policy": {
    "type": "k8s",
    "k8s_api_root": "https://11.11.11.120:6443",
...
#解压
mkdir /Containerd
tar -zxf containerd-1.7.3-linux-amd64.tar.gz -C /Containerd/
...
# [Install]
# WantedBy=multi-user.target
# 定义了服务的安装位置。这里指定为multi-user.target，表示将服务安装为多用户模式下的启动生成私钥：生成私钥：
```
    ...
    [plugins."io.containerd.grpc.v1.cri".registry]
      ...
      [plugins."io.containerd.grpc.v1.cri".registry.mirrors]
        [plugins."io.containerd.grpc.v1.cri".registry.mirrors."docker.io"]
          endpoint = [
            "https://9a988a342caa4702b5cda4470d46f83f.mirror.swr.myhuaweicloud.com",
...
#解压
mkdir /crictl
tar xf crictl-v*-linux-amd64.tar.gz -C /crictl
cp /crictl/* /usr/bin/
#生成配置文件
...
# 解压etcd安装文件
tar -zxf etcd-v3.5.9-linux-amd64.tar.gz && mv etcd-*/etcd /usr/local/bin/ && mv etcd-*/etcdctl /usr/local/bin/
...
cfssl gencert \
   ...
   -config=/k8s/k8s-ca/ca-config.json \
   -hostname=127.0.0.1,k8s-master01,k8s-master02,k8s-master03,11.11.11.120,11.11.11.223,11.11.11.184 \
   -profile=kubernetes \
...
mkdir /k8s/k8s-apiserver
cd /k8s/k8s-apiserver
...
cfssl gencert \
  ...
  -config=/k8s/k8s-ca/ca-config.json \
  -hostname=10.96.0.1,11.11.11.111,127.0.0.1,kubernetes,kubernetes.default,kubernetes.default.svc,kubernetes.default.svc.cluster,kubernetes.default.svc.cluster.local,x.oiox.cn,k.oiox.cn,l.oiox.cn,o.oiox.cn,11.11.11.120,11.11.11.223,11.11.11.29,11.11.11.148,11.11.11.175,11.11.11.176,11.11.11.177,11.11.11.178,11.11.11.179 \
  -profile=kubernetes \
  /k8s/k8s-apiserver/apiserver-csr.json | cfssljson -bare /etc/kubernetes/pki/apiserver
```
...
## 生成k8s-controller-manage的证书

### 生成master01认证加密文件

```apl
mkdir /k8s/k8s-controller-manager
cd /k8s/k8s-controller-manager
...
### 生成k8s-Scheduler 加密证书信息

```apl
mkdir /k8s/k8s-scheduler
cd /k8s/k8s-scheduler
...
cat > admin-csr.json << EOF 
{
  ...
  "hosts": [
    ...
    "admin",
    "admin.kubernetes",
    "admin.kubernetes.default",
    "admin.kubernetes.default.svc",
    "admin.kubernetes.default.svc.cluster",
    "admin.kubernetes.default.svc.cluster.local"
  ]
}
EOF
...
### 创建k8s-kube-Proxy加密证书信息

```apl
mkdir /k8s/k8s-kube-proxy
cd /k8s/k8s-kube-proxy
...
## 创建Service Account Key ——secret

> master01执行

```apl
mkdir /k8s/Service-Account-Key-secret
cd /k8s/Service-Account-Key-secret/
...
listen-peer-urls: 'https://11.11.11.120:2380'
listen-client-urls: 'https://11.11.11.120:2379,http://127.0.0.1:2379'
max-snapshots: 3
max-wals: 5
cors:
initial-advertise-peer-urls: 'https://11.11.11.120:2380'
advertise-client-urls: 'https://11.11.11.120:2379'
discovery:
discovery-fallback: 'proxy'
discovery-proxy:
discovery-srv:
initial-cluster: 'k8s-master01=https://11.11.11.120:2380,k8s-master02=https://11.11.11.223:2380,k8s-master03=https://11.11.11.184:2380'
...
election-timeout: 1000
quota-backend-bytes: 0
listen-peer-urls: 'https://11.11.11.223:2380'
listen-client-urls: 'https://11.11.11.223:2379,http://127.0.0.1:2379'
max-snapshots: 3
max-wals: 5
cors:
initial-advertise-peer-urls: 'https://11.11.11.223:2380'
advertise-client-urls: 'https://11.11.11.223:2379'
discovery:
discovery-fallback: 'proxy'
discovery-proxy:
discovery-srv:
...
election-timeout: 1000
quota-backend-bytes: 0
listen-peer-urls: 'https://11.11.11.184:2380'
listen-client-urls: 'https://11.11.11.184:2379,http://127.0.0.1:2379'
max-snapshots: 3
max-wals: 5
cors:
initial-advertise-peer-urls: 'https://11.11.11.184:2380'
advertise-client-urls: 'https://11.11.11.184:2379'
discovery:
discovery-fallback: 'proxy'
discovery-proxy:
discovery-srv:
...
etcdctl --endpoints="11.11.11.184:2379,11.11.11.223:2379,11.11.11.120:2379" --cacert=/etc/kubernetes/pki/etcd/etcd-ca.pem --cert=/etc/kubernetes/pki/etcd/etcd.pem --key=/etc/kubernetes/pki/etcd/etcd-key.pem  endpoint status --write-out=table
+-------------------+------------------+---------+---------+-----------+------------+-----------+------------+--------------------+--------+
|     ENDPOINT      |        ID        | VERSION | DB SIZE | IS LEADER | IS LEARNER | RAFT TERM | RAFT INDEX | RAFT APPLIED INDEX | ERRORS |
+-------------------+------------------+---------+---------+-----------+------------+-----------+------------+--------------------+--------+
| 11.11.11.184:2379 | 43f8837d1b3024bf |   3.5.9 |   20 kB |     false |      false |         2 |          8 |                  8 |        |
| 11.11.11.223:2379 | dbb2197233e8b685 |   3.5.9 |   20 kB |     false |      false |         2 |          8 |                  8 |        |
| 11.11.11.120:2379 | 8a474fd0560d6963 |   3.5.9 |   20 kB |      true |      false |         2 |          8 |                  8 |        |
+-------------------+------------------+---------+---------+-----------+------------+-----------+------------+--------------------+--------+


# 这个命令是使用etcdctl工具，用于查看指定etcd集群的健康状态。下面是每个参数的详细解释：
# 
# - `--endpoints`：指定要连接的etcd集群节点的地址和端口。在这个例子中，指定了3个节点的地址和端口，分别是`11.11.11.184:2379,11.11.11.223:2379,11.11.11.120:2379`。
...
stream {
    upstream backend {
        ...
        server 11.11.11.120:6443        max_fails=3 fail_timeout=30s;
        server 11.11.11.223:6443        max_fails=3 fail_timeout=30s;
        server 11.11.11.184:6443        max_fails=3 fail_timeout=30s;
    }
    server {
...
ExecStart=/usr/local/bin/kube-apiserver \\
      ...
      --secure-port=6443  \\
      --advertise-address=11.11.11.120 \\
      --service-cluster-ip-range=10.96.0.0/12,fd00:1111::/112  \\
      --service-node-port-range=30000-32767  \\
      --etcd-servers=https://11.11.11.120:2379,https://11.11.11.223:2379,https://11.11.11.184:2379 \\
...
ExecStart=/usr/local/bin/kube-apiserver \\
      ...
      --bind-address=0.0.0.0  \\
      --secure-port=6443  \\
      --advertise-address=11.11.11.223 \\
      --service-cluster-ip-range=10.96.0.0/12,fd00:1111::/112  \\
      --service-node-port-range=30000-32767  \\
      --etcd-servers=https://11.11.11.120:2379,https://11.11.11.223:2379,https://11.11.11.184:2379 \\
      --etcd-cafile=/etc/etcd/ssl/etcd-ca.pem  \\
...
ExecStart=/usr/local/bin/kube-apiserver \\
      ...
      --bind-address=0.0.0.0  \\
      --secure-port=6443  \\
      --advertise-address=11.11.11.184 \\
      --service-cluster-ip-range=10.96.0.0/12,fd00:1111::/112  \\
      --service-node-port-range=30000-32767  \\
      --etcd-servers=https://11.11.11.120:2379,https://11.11.11.223:2379,https://11.11.11.184:2379 \\
      --etcd-cafile=/etc/etcd/ssl/etcd-ca.pem  \\
...
- `--advertise-address=11.11.11.120` 广告API Server的地址。
- `--service-cluster-ip-range=10.96.0.0/12,fd00:1111::/112` 指定服务CIDR范围。
- `--service-node-port-range=30000-32767` 指定NodePort的范围。
- `--etcd-servers=https://11.11.11.120:2379,https://11.11.11.223:2379,https://11.11.11.184:2379` 指定etcd服务器的地址。
...
## 编写bootstrapping.yaml

```apl
cat > ~/bootstrap/bootstrap.secret.yaml << EOF
apiVersion: v1
kind: Secret
metadata:
...
ExecStart=/usr/local/bin/kubelet \\
    ...
    --container-runtime-endpoint=unix:///run/containerd/containerd.sock  \\
    --node-labels=node.kubernetes.io/node=  \\
    --node-ip=11.11.11.120,11.11.11.223,11.11.11.184,11.11.11.29,11.11.11.148
[Install]
WantedBy=multi-user.target
EOF
```
...
> 
> 或
> https://github.com/projectcalico/calico/blob/v3.27.2/manifests/custom-resources.yaml
> 
...
>
> ```apl
> ctr -n k8s.io images import --base-name docker.io/calico/cni:v3.27.2 calico-cni.tar
> ctr -n k8s.io images import --base-name docker.io/calico/node:v3.27.2 calico-node.tar 
> ctr -n k8s.io images import --base-name docker.io/calico/kube-controllers:v3.27.2 calico-kube-controllers.tar 
> ```
>
> 具体解释如下：
>
> - `ctr`: 是 Containerd 命令行工具的名称，用于与 Containerd 进行交互。
> - `-n k8s.io`: 是指定使用的命名空间。在这里，命名空间被设置为 "k8s.io"，表示操作将在该命名空间中进行。
> - `images import`: 是告诉 Containerd 导入镜像的命令。
> - `--base-name docker.io/calico/cni:v3.26.0`: 是指定要导入的镜像的基本名称和标签。在这里，镜像的基本名称是 "docker.io/calico/cni"，标签是 "v3.26.0"。这意味着要导入的镜像是 Calico CNI 的特定版本。
> - `calico-cni.tar`: 是要导入的镜像的存储路径和文件名。在这里，镜像将从名为 "calico-cni.tar" 的文件中导入。
>
> 该命令的目的是将 Calico CNI 镜像导入到 Containerd 中，以便在 Kubernetes 集群中使用 Calico 网络插件。导入镜像后，可以使用该镜像来创建和运行 Calico CNI 容器，提供网络功能和服务。

>修改calico配置文件（这里非常容易做错，在下边没有running就是这里的问题）一定不要多加空格，并且要对其

```apl
vim calico.yaml
            - name: CALICO_IPV4POOL_CIDR
              value: "172.16.0.0/12"  #位于4801行
```

部署Calico网络单节点配置（master）
...
>> [root@server1 opt]# kubectl get nodes
>> NAME      STATUS   ROLES           AGE   VERSION
>> server1   Ready    control-plane   15m   v1.27.1
>> server2   Ready    <none>          15m   v1.27.1
>> server3   Ready    <none>          15m   v1.27.1
>
>>[root@server1 opt]# kubectl get pod -n kube-system
>>NAME                                       READY   STATUS    RESTARTS   AGE
>>calico-kube-controllers-786b679988-w8j76   1/1     Running   0          16m
>>calico-node-b7q4c                          1/1     Running   0          16m
>>calico-node-qt94p                          1/1     Running   0          16m
>>calico-node-zcdgg                          1/1     Running   0          16m
>>coredns-7bdc4cb885-7h9rd                   1/1     Running   0          16m
>>coredns-7bdc4cb885-tzhm7                   1/1     Running   0          16m
>>etcd-server1                               1/1     Running   6          16m
>>kube-apiserver-server1                     1/1     Running   6          16m
>>kube-controller-manager-server1            1/1     Running   8          16m
>>kube-proxy-fjm6w                           1/1     Running   0          16m
>>kube-proxy-slzs6                           1/1     Running   0          16m
>>kube-proxy-sm77s                           1/1     Running   0          16m
>>kube-scheduler-server1                     1/1     Running   8          16m
...
## 每个节点都要能访问Kubernetes的SVC 443和dns的service 53
...
# 创建token
[root@k8s-master01 lzb]# kubectl -n kube-system create token admin-user

eyJhbGciOiJSUzI1NiIsImtpZCI6IkpSUS1lTjZ4LWdTUk9pOFRjeldYSmlkdFNIb3Y4ZzllTHUzcGdyM2xwS0UifQ..Q5mJN7sEitifhrtC2f0ySKpOLOO8QY7hVZvkgJfEJt1fWdWJUEoYCIDiiVfgUIgwtvncSIN5oVM0Qv1DI-YJzx-_DGkpzXSsl5n_whYz207BrWx0oDxHWnk5gYntYwecx7s0J1AUxwP2uFVC-fJqjzVcDCv9zdjZLwW9Cvjs2WDripEJvNJvOQPh4d4mcccSZTtX-Q2VyVpq0R5ys-CYKyTBBu_qZrc0TwddGNiFlsLG8c8bHwfifyHyaDXa9GY0pSdoVtYzalXdBGAS3VNcs2NtIZRVwbl9Q8oNHNbgxYO5jPwiKS_uZvpVknUTNuIsr15X3J2Y6GAoJJCuMmxRRw
```

![image-20241130125101541](https://github.com/liuzhenhua1223/2024-10-image/blob/master//computernetworks/image-20241130125101541.png?raw=true)

```apl
https://11.11.11.120:30504/#/workloads?namespace=default
```
...
ingress-host-bar   nginx   hello.chenby.cn,demo.chenby.cn   11.11.11.223   80      7s
...
![image-20241130175834511](https://github.com/liuzhenhua1223/2024-10-image/blob/master//computernetworks/image-20241130175834511.png?raw=true)
...
# 进行解析
[root@k8s-master01 11-30-pod-test]# kubectl exec busybox -n default -- nslookup coredns.kube-system
Server:    10.96.0.10
...
calico-node-97m55                          1/1     Running   0          38m     11.11.11.29     k8s-node01     <none>           <none>
calico-node-hlz7j                          1/1     Running   0          38m     11.11.11.223     k8s-master02   <none>           <none>
calico-node-jtlck                          1/1     Running   0          38m     11.11.11.184     k8s-master03   <none>           <none>
calico-node-lxfkf                          1/1     Running   0          38m     11.11.11.148     k8s-node02     <none>           <none>
calico-node-t667x                          1/1     Running   0          38m     11.11.11.120     k8s-master01   <none>           <none>
calico-typha-59d75c5dd4-gbhfp              1/1     Running   0          38m     11.11.11.148     k8s-node02     <none>           <none>
...
/ # ping 11.11.11.29
PING 11.11.11.29 (11.11.11.29): 56 data bytes
64 bytes from 11.11.11.29: seq=0 ttl=63 time=0.358 ms
64 bytes from 11.11.11.29: seq=1 ttl=63 time=0.668 ms
64 bytes from 11.11.11.29: seq=2 ttl=63 time=0.637 ms
64 bytes from 11.11.11.29: seq=3 ttl=63 time=0.624 ms
64 bytes from 11.11.11.29: seq=4 ttl=63 time=0.907 ms
